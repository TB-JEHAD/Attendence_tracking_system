    ...
    <div class="glass-card">
        ...
        @guest
            <div class="cta-buttons d-flex flex-wrap justify-content-center">
                <a href="{{ route('login') }}" class="btn btn-outline-glow">🔐 Login</a>
                <a href="{{ route('register') }}" class="btn btn-glow">✍️ Register</a>
            </div>
        @endguest
        ...
        @auth
            <div class="alert-box mt-4">
                👋 Hello, <strong>{{ Auth::user()->name }}</strong>! You're already logged in.
                <div class="mt-2">
                    <a href="{{ url('/redirect') }}" class="btn btn-glow btn-sm mt-2">Go to Dashboard →</a>
                </div>
            </div>
        @endauth
    </div>
